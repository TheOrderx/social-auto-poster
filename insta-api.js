...
function downloadWithYtDlp(targetUrl, sessionId) {
    return new Promise((resolve, reject) => {
        ...
        // -o: Çıktı dosyası, --no-progress: Log taşmasını önler
        const cmd = `${binPath} -o "${outPath}" --no-progress --no-playlist --user-agent "Mozilla/5.0" "${targetUrl}"`;
...
app.get('/download', async (req, res) => {
    ...
    const cleanUrl = (isYouTube && url.includes("watch?v=")) ? url.split('&')[0] : url.split('?')[0];
    ...
    try {
        // --- TikTok & YouTube: Bağımsız yt-dlp (Doğrudan İndirme) ---
        if (isTikTok || isYouTube) {
            console.log(`🔍 yt-dlp (Bağımsız Mod) deneniyor...`);
            try {
                const videoPath = await downloadWithYtDlp(url, sessionId);
                if (videoPath) {
                    console.log(`✅ yt-dlp: Video doğrudan indirildi.`);
        ...
        if (isYouTube) {
            const cobaltRes = await axios.post('https://api.cobalt.tools/api/json', { url: cleanUrl, vCodec: 'h264' }, { timeout: 15000 });
