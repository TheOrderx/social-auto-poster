...
app.get('/login', (req, res) => {
    ...
    url += '&scope=user.info.basic,user.info.profile,user.info.stats,video.list,video.upload';
...
app.get('/receive-code', async (req, res) => {
    ...
    try {
        ...
        if (token) {
            ...
            // Regex ile TIKTOK_ACCOUNTS içindeki token değerini bul ve değiştir
            const regex = /(TIKTOK_ACCOUNTS=\[.*?\"token\"\s*:\s*\")[^\"]+(\".*?\])/;
            envContent = envContent.replace(regex, `$1${token}$2`);
